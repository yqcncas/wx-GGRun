<template>
	<view class="storage">
		...
		<view class="textArea-box"   v-if="scrollIndex === 2" style="padding-top: 30rpx;">
			<textarea :show-confirm-bar = "false" value="" placeholder="输入你想要买的商品，比如：红牛一厅" class="helpBuyTextArea"  @blur = "setHelpBuyInfo"  v-model="helpBuyTextArea"/>
 					...
 					<view class="address-left">
						...
						<view class="address-center" @tap="goToEdit(sendAddress, 0, sendAddress.id)" v-else :class="{'bdr' : scrollIndex === 2 }">
							...
							<view class="center-top" v-if="assignIndex === 1">
								...
								<view class="user-name" v-else-if="sendAddress.addressDetail[2]">{{ sendAddress.addressDetail[2] }}</view>
								<view class="user-name" v-else>请指定地址</view>
							</view>
			...
			<view class="storage-item-box" :class="{changebdPad:scrollIndex === 2}">
				<view class="storage-item baojia" @tap="pickTimer" v-if="scrollIndex != 2">
					...
					<view class="item-left">
						...
						<view class="item-left-top"><image src="../../static/image/order/qujiantimer.png" mode=""></image>取件时间</view>
						<view class="item-left-bottom">骑手会在30分钟内上门取件</view>
					</view>
					<view class="item-right">
						<view>{{ showChoiceTime ? showChoiceTime : '马上取件' }}</view>
						<image src="../../static/image/order/right.png" mode="aspectFill"></image>
					</view>
				</view>
				...
				<view class="storage-item baojia" @tap="goToShopInfo" v-else>
					...
					<view class="item-left">
						...
						<view class="item-left-top"><image src="../../static/image/order/shopinfo.png" mode=""></image>商品信息</view>
				...
				<view class="storage-item baojia" @tap="goToDiscount">
					...
					<view class="item-left">
						...
						<view class="item-left-top"><image src="../../static/image/order/youhui.png" mode=""></image>优惠券</view>
				...
				<view class="storage-item baojia" v-if="scrollIndex != 2">
					<view class="item-left">
						...
						<view class="item-left-top"><image src="../../static/image/order/baojia.png" mode=""></image>保价服务（¥{{ orderProperty }}）</view>
				...
				<view class="storage-item jiaji" v-if="rightIndex === 999">
					<view class="item-left">
						<view class="item-left-top"><image src="../../static/image/order/jiaji.png" mode=""></image>加急专送</view>
				...
				<view class="storage-item" v-if="scrollIndex === 2">
					<view class="item-left"><image src="../../static/image/order/qianshou.png" mode=""></image>签收码</view>
				...
				<view class="storage-item">
					...
					<view class="item-left" >
						...
						<view class="item-left-top"><image src="../../static/image/order/redbag.png" mode=""></image>打赏小费</view>
		...
		<view class="footer-left">
			<view class="fl-top">
				...
				<view class="flt-right" @tap="goTofreight(0)">
					...
					<image src="../../static/image/order/up.png" mode="aspectFill"></image>
				</view>
			</view>
	...
	<view class="storage-footer column-footer" v-else>			<view class="newpopups-footer-top">				<view class="newpopups-footer-top-l">					<view class="paotuifei">跑腿费: <span>￥{{orderComputed.taskReward ? orderComputed.taskReward : 0}}</span></view>					<view class="paotuifei">押金: <span>￥{{orderComputed.goodsPredictAmount ? orderComputed.goodsPredictAmount : 50}}</span></view>				</view>				<view class="newpopups-footer-left-top-c">					合计: <span>￥{{orderComputed.payAmount ? orderComputed.payAmount : 50}}</span>				</view>				<view class="newpopups-footer-left-top-r" @tap="handleBilldetail">					确认下单				</view>			</view>			<view class="newpopups-footer-bottom">				<view class="newpopups-footer-bottom-l" @tap="choiceHelpbuyCircle">			
		...
		<view class="horseman-box" v-if="horseman">
			<view class="horseman-item">
				<view class="close" @tap="handleHorsemanShow"><image src="../../static/image/order/close.png" mode="aspectFill"></image></view>
				<view class="horseman-box-center">
					<view class="center-header"><image src="../../static/image/order/zhuyi.png" mode="aspectFill"></image></view>
		...
		<view class="shopAppraisement" v-show="shopAppraisement" >
			<view class="shopAppraisement-wrapper" :class="{newTop: platform == 'ios'}">
				<view class="shopAppraisement-header" @click.stop="closeShopAppraisement">
					<view>填写商品估价</view>
					<image src="../../static/image/order/closeBlack.png" mode="aspectFill" ></image>
				</view>
				...
				<scroll-view scroll-y="true">
					<view class="right-timer-box">
						<view class="timer-item takenow" v-if="dayIndex === 0" @tap="initTakeNow">
							...
							<image src="../../static/image/order/duihou.png" mode="aspectFill" v-if="rightIndex === 999"></image>
						</view>
						<view class="timer-item" v-for="(item, index) in timeList" :key="index" @tap="handleRight(item, index)" v-if="dayIndex === 0">
							...
							<image src="../../static/image/order/duihou.png" mode="aspectFill" v-if="index === rightIndex"></image>
						</view>
				...
				<view class="popups-footer-left">
					<view class="footer-left-top">
						...
						<view class="flt-right" @tap="handleDetailshow">
							...
							<image src="../../static/image/order/up.png" mode="aspectFill" class="up-down" :class="{ on: priceDetailshow }"></image>
						</view>
					</view>
	...
	methods: {
				...
				if (this.rightIndex === 999) {
					...
					if (JSON.stringify(this.discount) === '{}') {
						if (this.scrollIndex === 2 && this.helpBuy === 0) {
							...
							if (this.scrollIndex == 2 && this.assignIndex == 1) {
								let res = await this.$fetch(
									this.$api.orderCreate,
									{
										buyAddressType: 0,
		...
		async payOrder() {
			...
			if (this.circleShow === 0) {
				let res = await this.$fetch(this.$api.orderPay, { orderId: this.orderId, payType: 0 }, 'POST', 'form');
				...
				if (res.code == 0) {
					...
					setTimeout(() => {
						...
						if (this.$dayjs().unix() < this.$dayjs('2020-05-28').unix()) {
							uni.reLaunch({
								url: '../index/index'
							})
						} else {
							uni.reLaunch({
								url: '../SuccessOrder/SuccessOrder?orderId=' + this.orderId
							})
						}
				...
				if (uni.getStorageSync('openId')) {
					var res = await this.$fetch(this.$api.orderPay, { orderId: this.orderId, payType: 4, openId: uni.getStorageSync('openId') }, 'POST', 'form');
					
				} else {
					var res = await this.$fetch(this.$api.orderPay, { orderId: this.orderId, payType: 4 }, 'POST', 'form');
				}
		...
		.newpopups-footer-bottom{
			...
			.newpopups-footer-bottom-l{
				...
				.horseman-circle {
					...
					box-sizing: border-box;
					&.on {
						background: url(../../static/image/order/choice.png) no-repeat;
